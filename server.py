...
#打开数据库连接
db = pymysql.connect(host='43.138.9.135', port=3306, user='liang', passwd='liangfengdi175x', db='file_sys',\
                     charset='utf8')
#使用cursor方法创建一个游标
cursor = db.cursor()
#查询数据库版本
...
class server_ssl:
    def server_listen(self):
        ...
        # 加载信任根证书
        context.load_verify_locations('./ca_final/ca/ca.crt')
        # 加载服务器所用证书和私钥
        context.load_cert_chain('./ca_final/server/server.crt', './ca_final/server/server-key.pem')
        # 双向校验模式
